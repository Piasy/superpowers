...
**目的：** 用一个 subagent 顺序完成 `controller` 开发分支收尾验证：完整测试、关键运行时 smoke check、最终全量实现 review。
...
**发现问题时：** 返回失败 verdict；不要自行修复。Controller 会关闭你，并派发新的 `xhigh` 收尾修复 subagent 在 `controller` 开发分支修复；修复后再派发新的 fresh 收尾验证 subagent。

建议由 controller 提供以下上下文占位符：
- `SPEC_REFERENCE` - 当前轮实现对应的 spec 文件路径（必要时含父/子 spec 路径）
- `IMPLEMENTED_SLICES` - 当前已标记为 `Implementation status: Done` 的 feature slice 列表（含 AC 引用）
- `OUT_OF_SCOPE_SLICES` - 仍为 `Implementation status: Not done` 的 slice、其他未实现子 spec，以及 `Candidate Future Split Specs`
- `CRITICAL_ENTRYPOINTS` - 需要在整体集成后仍然保持正确的关键公共入口，以及 spec/AC 要求的 trigger-driven workflow
- `VERIFICATION_EXPECTATIONS` - 完整测试命令与关键 smoke check 预期
- `BASE_SHA` - 本轮整体实现开始前的 commit，或与 base branch 的 merge-base
- `HEAD_SHA` - 当前 `controller` 开发分支 head
- `DESCRIPTION` - 当前整体实现摘要

```
派发一个 fresh subagent，使用下面的 prompt：
  description: "验证 controller 分支收尾"
  prompt: |
    你正在验证已经集成到 controller 开发分支上的最终全量实现。

    ## 关键职责

    这是一次整体收尾 gate。
    不要把它降级成普通 diff review，也不要把单个任务已经通过 review 当作整体无问题的证明。
    如果完整测试或 smoke check 发现阻塞问题，返回 Issues Found，不要继续最终全量实现 review。
    对于因前置步骤失败而未执行的后续输出项，明确标记 `Not run` 并说明被哪个步骤阻塞。

    ## 执行步骤

    **第 1 步：验证完整测试**
    在 `controller` 开发分支上验证完整项目测试套件通过，并报告实际命令、退出码、失败数量和关键失败内容。使用 `VERIFICATION_EXPECTATIONS` 中的项目命令；未提供时按项目惯例选择，例如：

    ```bash
    npm test / cargo test / pytest / go test ./...
    ```

    如果测试失败，返回 Issues Found。不要自行修复。

    **第 2 步：验证关键运行时路径**
    对 spec/AC 要求的每个关键 trigger-driven workflow，按运行时语义 gate 至少跑一次公共入口 smoke check。没有这类 workflow 时，明确报告跳过原因；如果 smoke check 暴露出“只有状态变化，没有真实执行”，返回 Issues Found。

    **第 3 步：最终全量实现 Review**
    把已经集成好的 `controller` 开发分支当作整体检查：
    ...
    **第 4 步：形成完成 verdict**
    只有第 1、2、3 步都通过，才能返回 Approved。返回 verdict 后等待 controller 记录并关闭你；不要自行决定或执行后续分支处置。
    ...
    **Head:** [HEAD_SHA]
    ...
    你必须直接阅读集成后的代码，并把整体结果与已批准 spec 对照。
    ...
    **Full Test Verification:**
    - [实际命令、退出码、通过/失败结果；失败时列出失败数量和关键失败内容]

    **Smoke Check Verification:**
    - [每个关键 trigger-driven workflow 的 smoke check 结果，或明确跳过原因]
...
**Subagent 返回：** `Status`、`Full Test Verification`、`Smoke Check Verification`、`Done Slice Coverage`、`Out-of-Scope Audit`、`Integration Findings`、`Runtime Semantics Findings`、`Issues`、`Assessment`。
